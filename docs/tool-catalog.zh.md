...
| `@deepseek-ai/dsh-tool-model-switch` | `list_models`、`switch_model` | `ctx.tools`、`sessionController 服务（调用时惰性查找）`、`switch_model 需要一个调用方 Agent 会话` | `model/selection`、`agent-default-model`、`tool/call`、`tool/result` | - | 两个工具都由 agent 预设挂载，并委托给 Web 面提供的 `sessionController` 服务。在没有会话控制器的部署（headless 或 SDK）中，工具保持可见，但在调用时以固定错误失败；在这种模式下，调用是唯一能发现该路由不可用的方式。 |
...
列出此部署可路由到的 LLM 提供方与模型、当前默认选择，以及目录加载失败的提供方。调用 switch_model 前先调用它。
...
来源：[`packages/llm/tool-model-switch/src/index.ts`](../packages/llm/tool-model-switch/src/index.ts)

### `switch_model`

切换本会话运行的 LLM 模型。切换从 agent 的下一次模型请求生效，并记录在会话日志中。新会话的部署默认值也会更新为所选路由。先调用 list_models 查看可用的提供方与模型。需要为本地任务释放 GPU 内存时很有用：切换到更轻或不同位置的模型，完成任务后再切换回来。
...
两个工具都由 agent 预设挂载，并委托给 Web 面提供的 `sessionController` 服务。在没有会话控制器的部署（headless 或 SDK）中，工具保持可见，但在调用时以固定错误失败；在这种模式下，调用是唯一能发现该路由不可用的方式。
