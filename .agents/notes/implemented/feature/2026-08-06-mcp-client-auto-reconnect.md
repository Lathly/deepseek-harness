...
**Trigger.** The supervisor arms `client.onclose` per generation. The SDK fires it when the stdio child exits, so a crash is observed without polling. A streamable-http connection ends the same way: the transport's `onclose` on any close, plus a network-level mid-call failure that the tool executor reports into the supervisor (the [transport failure regeneration](2026-09-02-mcp-client-transport-failure-regeneration.md) note owns that report), so an HTTP row that breaks inside a tool call regenerates through the same cycle instead of wedging.
...
**Route Streamable HTTP request failures into the supervisor.** Rejected at the time: the HTTP transport already reconnects its SSE stream with its own backoff, per-request errors do not imply a dead server, and there is no child process the harness could respawn. Transport close stayed the single trigger until a silent socket cut wedged an established connection — since adopted, scoped to network-level failures, by the [transport failure regeneration](2026-09-02-mcp-client-transport-failure-regeneration.md) note.
